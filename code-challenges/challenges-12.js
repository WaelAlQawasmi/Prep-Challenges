...
const cvsFiltration = (arr) => {
  let  retArr=[];
 let newArr=arr.filter(cv =>(cv.tech=="JS"&&cv.yearsOfExperience>4));
...
return arr.filter(x =>!/a|e|i|o|u/g.test(x));}
...
const skillComparison = (arr1, arr2) => {
    let arrE1=[];
    let arrE2=[];
    arrE1= arr1.filter(x=>!arr2.includes(x)
    
    )

    arrE2=arr2.filter(x=>!arr1.includes(x)
    
    )
    arrE2.forEach(element => {
        arrE1.push(element);  
