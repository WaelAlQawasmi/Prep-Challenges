...
function fullName(arr) {
    return arr.map((element) => {return `${element.firstName} ${element.lastName}`;});
...
function gradesAvg(arr) {
    ...
    {  let avg=0;
        element.gradsList.map((element1)=>{avg= element1+avg;
         return avg;

    } )
    element["avg"]=avg/element.gradsList.length;
...
function studentsResult(arr) {
    let returnArr = gradesAvg(arr);
    returnArr.map((x)=> { if(x.avg >= 50.0) {
        x["result"] = "Passed" ;
    }
    else 
     x["result"] = "Failed"; } );
